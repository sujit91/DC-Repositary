...
public class Verify_Change_Password_by_providing_the_new_password extends TestBase{
	  String sFilePath = System.getProperty("user.dir")+"\\TestData\\Login\\Login.xlsx";
	  boolean Report=false;
	   
	@Test(description="Verify Change Password (by providing the new password which doesn’t meet the required criteria)")
	public void VerifyChange_Password_by_providing_the_new_password() throws Throwable{
		try {
			...
			String newpassword=Utility.Random_StringGenerator(3)+Utility.Random_StringGeneratorLowerCase(5);
			login.enterChangePasswordDetails(ObjectReader.reader.getPassword(), newpassword, newpassword);
			...
			CustomizeReport( "Verify_Change_Password_by_providing_the_new_password", "pass", "Verify Change Password (by providing the new password which doesn’t meet the required criteria)");
			Report=true;
		} finally {
			if (Report==false) {
			CustomizeReport("Verify_Change_Password_by_providing_the_new_password", "fail", "Verify Change Password (by providing the new password which doesn’t meet the required criteria)");	
			}
		}
		}
