...
public class search_for_a_customer_using_RA_Number extends TestBase{
    String sFilePath = System.getProperty("user.dir")+"\\TestData\\CustomerSearch\\CustomerSearch.xlsx";
    String testscenario="search_for_a_customer_using_RA_Number";
    HashMap<Object, Object> keyvalue = new HashMap<Object, Object>();
    
    @Test(description="search_for_a_customer_using_RA_Number")
    public void CS_028() throws Throwable{
		...
		try {	
			...
			//====fatch db details from sql server for a RA_Number number=======
			...
			String RA_Number=Dbscript.EmailValidationsSQL(dbQuery, columnName);	
			//===remove first 2 character=======================
			String RA=Utility.removeCharacter(RA_Number).trim();		
			...
			dashboard.provideSearchFieldValueAndClickSearch(RA);
		    ...
		    CustomizeReport("search_for_a_customer_using_RA_Number", "pass", "search_for_a_customer_using_RA_Number");
		    Report=true;
	} finally {
		if (Report==false) {
			CustomizeReport("search_for_a_customer_using_RA_Number", "fail", "search_for_a_customer_using_RA_Number");	
		}
	}
